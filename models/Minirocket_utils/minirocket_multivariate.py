'''
- the following code is mainly based on the original MiniROCKET implementation from Dempster https://github.com/angus924/minirocket
- we just added the HDC operation
'''
...
class MiniRocketMultivariate(_PanelToTabularTransformer):
    ...
    **Multivariate**
    ...
    def transform(self, X, y=None, use_hdc=False):
        ...
        X = check_X(X, coerce_to_numpy=True).astype(np.float32)
        results = _transform_multi(X, self.parameters, self.poses, use_hdc=use_hdc)
...
@njit(
    "float32[:,:](float32[:,:,:],Tuple((int32[:],int32[:],int32[:],int32[:],float32[:])),float32[:,:],bool_)",  # noqa
    fastmath=True,
    parallel=True,
    cache=True,
)
def _transform_multi(X, parameters, poses, use_hdc=False):
    poses = poses.ravel()
    n_instances, n_columns, n_timepoints = X.shape
